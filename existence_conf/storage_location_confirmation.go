...
func (c *ExistenceConf) itemStorageLocationExistenceConf(mapper ExConfMapper, input *dpfm_api_input_reader.SDC, existenceMap *[]bool, exconfErrMsg *string, errs *[]error, mtx *sync.Mutex, wg *sync.WaitGroup, log *logger.Logger) {
	...
	for _, item := range items {
		bpID, plant, storageLocation, err := getItemStorageLocationExistenceConfKey(mapper, &item, exconfErrMsg)
		if err != nil {
...
func getItemStorageLocationExistenceConfKey(mapper ExConfMapper, item *dpfm_api_input_reader.Item, exconfErrMsg *string) (int, string, string, error) {
	var bpID int
	var plant string
	...
	case "DeliverToPlantStorageLocation":
		...
		if item.DeliverToPlant != nil ||
			...
			if len(*item.DeliverToPlant) == 0 ||
				len(*item.DeliverToPlantStorageLocation) == 0 {
				err = xerrors.Errorf("cannot specify null keys")
				return 0, "", "", err
			}
		}
	...
	case "DeliverFromPlantStorageLocation":
		...
		if item.DeliverFromPlant != nil ||
			...
			if len(*item.DeliverFromPlant) == 0 ||
				len(*item.DeliverFromPlantStorageLocation) == 0 {
				err = xerrors.Errorf("cannot specify null keys")
				return 0, "", "", err
			}
		}
	...
	case "ProductionPlantStorageLocation":
		...
		if item.ProductionPlant != nil ||
			item.ProductionPlantStorageLocation != nil {
			if len(*item.ProductionPlant) == 0 ||
				len(*item.ProductionPlantStorageLocation) == 0 {
				err = xerrors.Errorf("cannot specify null keys")
				return 0, "", "", err
			}
		}
	...
	return bpID, plant, storageLocation, nil
}
